...
int main(int argc, char **argv) {
    ...
    int sumx = 0, sumy = 0, area = 0; // area is number of white pixels
    for(int r = 0; r < binaryImage.num_rows(); r++){
      for(int c = 0; c < binaryImage.num_columns(); c++){
        int pixel = binaryImage.GetPixel(r, c); // white pixel
        if(pixel == 255){ // part of the object
          sumx += c;
          sumy += r;
          area++; // count white pixels
        }
      }
    }
    int centroidX = sumx / area; // centroid x
    int centroidY = sumy / area; // centroid y
    // radius
    int radius = 0;
    for(int r = 0; r < binaryImage.num_rows(); r++){
      for(int c = 0; c < binaryImage.num_columns(); c++){
        ...
        if(pixel == 255){ // part of the object
          ...
          if(dist > radius) // update radius
            radius = dist;
        }
      }
    }
    // write the output
    std::ofstream outputFile(output_file); // open the file
    if(!outputFile.is_open()){
      std::cout << "Could not write " << std::endl;
      return 1;
    }
    outputFile << centroidX << " " << centroidY << " "  << radius << std::endl; // write to file
    outputFile.close(); 
    std::cout << "Running s1 " << input_file << " " << threshold << " " << output_file << std::endl; // print 
    return 0;
}